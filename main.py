...
def KMeans(image: np.ndarray, k: int, mask: bool) -> np.ndarray:
    ...
    ret, label, centers = cv2.kmeans(
        image, k, None, condition, 10, cv2.KMEANS_RANDOM_CENTERS
    )
...
def morphImg(image: np.ndarray) -> np.ndarray:
    """
    Applies Inverse Otsu Thresholding and preset morphological operations on the image
    """
    ...
    # img = cv2.GaussianBlur(img, (3,qq 3), 0)
...
def img_prc(image: np.ndarray) -> np.ndarray:
    ...
    # MORPHOLOGICAL SEGMENTATION AND THRESHOLDING
    ...
    areas = [
        (cv2.contourArea(c), c)
        for c in contours
        if cv2.contourArea(c) < int(0.9 * img.shape[0] * img.shape[1])
    ]
